...
const CardTemp = ({ card, setActive, id }) => {
  return (
    ...
    >
      <div className="flex gap-4 flex-col md:flex-row overflow-hidden">
        <motion.div layoutId={`image-${card.title}-${id}`} className="shrink-0">
          <Image
            ...
            src={card.src}
            alt={card.title}
            className="size-40 md:h-30 md:w-30 rounded-lg object-cover object-top"
          />
        </motion.div>
        <div className="">
          <motion.h3
            layoutId={`title-${card.title}-${id}`}
            className="max-w-150 font-medium text-neutral-800 dark:text-neutral-200 text-2xl line-clamp-2"
          >
            {card.title}
          </motion.h3>
          <motion.p
            layoutId={`description-${card.description}-${id}`}
            className="px-2 mt-3 mx-2  w-9/10 text-neutral-600 dark:text-neutral-400 line-clamp-1"
          >
            {card.description}
          </motion.p>
        </div>
