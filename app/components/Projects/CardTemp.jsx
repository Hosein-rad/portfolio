...
const CardTemp = ({ card, setActive, id }) => {
  return (
    ...
    >
      ...
      <div className="w-full md:w-fit -mt-8 sm:-mt-3 lg:mt-0 flex flex-row md:flex-col gap-1 md:gap-2 items-start md:items-center">
        <motion.a
          href={card.ctaLink}
          target={card.ctaLink === "#" ? "" : "_blank"}
          layoutId={`button-${card.title}-${id}`}
          className="flex items-center justify-center rounded-full bg-blue-100 dark:bg-white size-7 md:size-10 hover:bg-sky-600 hover:text-white text-black mt-4 md:mt-0 duration-200 cursor-pointer shrink-0 group"
        >
          {card.ctaText}
        </motion.a>
        {card.ctaLink2 && (
